...
/*SET VALUE ------------------------------------------------------------------*/
void DjiUser_SetValue(E_DjiWidgetType widgetType, uint32_t index, int32_t value){
    if(index < s_widgetHandlerListCount){
        T_DjiReturnCode result = DjiTestWidget_SetWidgetValue(widgetType , index , value ,NULL);
        if(result != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS){
            USER_LOG_ERROR("Dji widget_value set error");
        }
    }
    else{
        USER_LOG_ERROR("Dji widge_value_index overflow");
